...
- D-017 (2026-09-17) P05 Media & Moderation Architecture: Client-side HTML5 canvas downsampling to 1600px with EXIF metadata stripping; private `listing-photos` bucket with time-bound signed URLs; `moderate_listing` RPC for campus staff; dual price filter semantics separating total sale price from daily rental rates.
- D-018 (2026-09-17) P06 & P07 Exchange & Direct Payment Architecture: Direct peer-to-peer payments outside platform with zero escrow/gateway claims; private `payment-proofs` storage bucket restricted to transaction participants and moderators; payer-only upload with max 3 revisions; payee-only acknowledgement and dispute; PostgreSQL exclusion constraint `no_overlapping_active_reservations` (`23P01`) for atomic concurrency conflict prevention on loan/rental windows.